...
def test_sync_and_thread_list(client):
    _sync(client)
...
def test_analyze_creates_deadline_task(client):
    _sync(client)
...
def test_reply_updates_thread_for_mail_ui(client):
    _sync(client)
    ...
    thread = client.get("/threads/thr_1001").json()
